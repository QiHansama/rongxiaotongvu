<template>
  <div class="receiving-address">
    ...
    <el-dialog title="更换地址" :visible.sync="dialogVisible" width="580px" :before-close="handleClose">
      <el-table class="passive-scroll" :data="allAddressData" tooltip-effect="dark" style="width: 100%"
...
import {addressAll1, addressUpdate} from "@/apis/address";
 ...
 /* mounted() {
  ...
  methods: {
    ...
    submitChange(){
      ...
      addressUpdate(this.selectedRow).then(res=>{
        if (res.code === 200) {
          this.$message({
            type: "success",
            message: res.message
...
.passive-scroll .el-table__body-wrapper {
  touch-action: pan-y;
}
</style>