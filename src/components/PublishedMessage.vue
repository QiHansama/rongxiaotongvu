      ...
      <div class="message" v-for="(item, index) in userGoods" :key="index">
        ...
        <div class="info">
          ...
          <div class="published-message-operation">
            <span @click.once="changeInfo(item)">
              <change-message
                  :cupdateGoodInfo="updateGoodInfo"
                  @update-success="findAll"
              ></change-message>
            </span>
            <span @click.stop="deleteInfo(item.orderId)">
              <delete-message @delete-success="findAll"></delete-message>
            </span>
          </div>
        </div>
      </div>
...
import {
  findOrderByUserIdAndGoods,
  findOrderByUserIdAndNeeds,
  findOrdersByLikeByGoods,
  findOrdersByLikeByNeeds
} from "@/apis/order";
...
export default {
  ...
  methods: {
    ...
    publishClick() {
      this.$router.push("/home/addMessage/publish" + this.ctype);
    },

    changeInfo(item) {
      this.$store.commit("updateOrder", item.orderId);
      this.updateGoodInfo={...item}
    },
    deleteInfo(item) {
    ...
    getData() {
      if (this.ctype === "needs") {
        ...
        findOrdersByLikeByNeeds(param).then(res => {
          if (res.code == 200) {
        ...
        findOrdersByLikeByGoods(param).then(res => {
          if (res.code == 200) {
            this.userGoods = res.data
            console.log(res.data)
            this.$message({
    ...
    findAll(){
      if(this.ctype=="goods"){
        findOrderByUserIdAndGoods(this.user.id).then(res=>{
          if(res.code==200){
            this.userGoods=res.data
          }
        })
      }else {
        findOrderByUserIdAndNeeds(this.user.id).then(res=>{
          if(res.code==200){
            this.userGoods=res.data
          }
        })
